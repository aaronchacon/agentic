...
describe('AgtApproval', () => {
  ...
  it('shows the action and approve/reject controls when pending', () => {
    const el = render({ action: 'Auto-approve case #1', detail: { risk: 'low' } })
      .nativeElement as HTMLElement;
    expect(el.querySelector('.agt-approval__action')?.textContent).toContain('Auto-approve case #1');
  ...
  it('emits approve and moves to the approved state', () => {
    ...
    (fixture.nativeElement.querySelector('.agt-approval__btn--approve') as HTMLButtonElement).click();
    fixture.detectChanges();
    const el = fixture.nativeElement as HTMLElement;
    expect(emitted).toBe(true);
    expect(fixture.componentInstance.state()).toBe('approved');
    expect(el.querySelector('.agt-approval__status--approved')?.textContent).toContain('Approved');
  ...
  it('emits reject and moves to the rejected state', () => {
    ...
    (fixture.nativeElement.querySelector('.agt-approval__btn--reject') as HTMLButtonElement).click();
    fixture.detectChanges();
    expect(emitted).toBe(true);
    expect(fixture.componentInstance.state()).toBe('rejected');
    expect(fixture.nativeElement.querySelector('.agt-approval__status--rejected')).toBeTruthy();
  });

  it('shows an Edit button when editable', () => {
    const el = render({ action: 'x', editable: true }).nativeElement as HTMLElement;
    expect(el.querySelector('.agt-approval__btn--ghost')?.textContent).toContain('Edit');
  });
});
