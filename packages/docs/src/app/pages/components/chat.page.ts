...
const reply: FixtureScript = [
  { type: 'reasoning', delta: 'Checking document authenticity, then screening sanctions and PEP lists.' },
  ...
  template: `
    <article class="doc">
      <div class="doc__head">
        ...
        <p class="doc__lead">
          A ChatGPT-style chat surface: streaming markdown with syntax-highlighted code blocks, a
          collapsible reasoning trace, a thinking indicator, an empty state with suggestions and an
      ...
      <section class="doc__section">
        <h2>Basic</h2>
        <p>
          The component is presentational: pass a store created with <code>injectAgent</code>. Connect
          your own transport (SSE/WebSocket) or replay fixtures — the kit never talks to an LLM.
        </p>
        <doc-demo [code]="code">
...
export default class ChatPage {
  protected readonly props: DocProp[] = [
    { name: 'store', type: 'AgentStore', required: true, description: 'Agent store created with injectAgent().' },
    { name: 'suggestions', type: 'string[]', default: '[]', description: 'Prompt chips shown in the empty state.' },
    { name: 'emptyTitle', type: 'string', default: "'What can I help with?'", description: 'Title shown in the empty state.' },
    { name: 'placeholder', type: 'string', default: "'Ask anything'", description: 'Composer placeholder text.' },
    { name: 'ariaLabel', type: 'string', default: "'Message'", description: 'Accessible label for the composer.' },
    { name: 'streamSpeed', type: "StreamSpeed", default: "'smooth'", description: "'instant' | 'slow' | 'smooth' | 'fast', or words/second." },
  ];

  protected readonly suggestions = ['Summarize this case', 'What documents are missing?'];
