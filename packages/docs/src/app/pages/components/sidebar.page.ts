  ...
  template: `
    <article class="doc">
      <div class="doc__head">
        ...
        <p class="doc__lead">
          A collapsible agent side panel that wraps its content (e.g. the chat). When closed it shows
          a floating action button with an optional badge; when open it slides in a docked panel with
          managed focus and <code>inert</code>.
        </p>
        <doc-md [markdown]="md" />
      </div>
      ...
      <section class="doc__section">
        <h2>Agent panel</h2>
        <p>
          The launcher is fixed to the bottom-right of the page. Click it to slide in the panel — the
          FAB colour is themeable via <code>--agt-sidebar-fab-background</code>.
        </p>
        <doc-demo [code]="code">
          <div class="sidebar-hint">The agent launcher is at the bottom-right of the page → open it.</div>
        </doc-demo>
      </section>
...
export default class SidebarPage {
  protected readonly props: DocProp[] = [
    { name: 'open', type: 'boolean', default: 'false', description: 'Whether the panel is open (two-way, [(open)]).' },
    { name: 'title', type: 'string', default: "'Agent'", description: 'Panel header title.' },
    { name: 'side', type: "'right' | 'left'", default: "'right'", description: 'Which edge the panel docks to.' },
    { name: 'badge', type: 'number | boolean', default: 'false', description: 'Unread badge on the launcher (count or dot).' },
  ];

  protected readonly suggestions = ['Why do you need this?', 'What documents are missing?'];
