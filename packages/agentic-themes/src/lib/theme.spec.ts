...
describe('resolveRefs', () => {
  ...
  it('kebab-cases camelCase segments', () => {
    expect(resolveRefs('{content.hoverBackground}')).toBe('var(--agt-content-hover-background)');
  });

  it('resolves multiple references inside one value (gradient)', () => {
    expect(resolveRefs('linear-gradient(90deg, {violet.500}, {cyan.500})')).toBe(
...
describe('buildThemeCss', () => {
  ...
  it('compiles the AI gradient with both palette references', () => {
    expect(css).toContain(
      '--agt-ai-gradient: linear-gradient(90deg, var(--agt-violet-500), var(--agt-cyan-500));',
    );
  });
  ...
  it('honours a custom dark selector', () => {
    expect(buildThemeCss(Base, { darkSelector: '.app-dark' })).toContain('.app-dark {');
  });
});
...
describe('definePreset', () => {
  ...
  it('does not mutate the base preset', () => {
    ...
    definePreset(Base, { semantic: { colorScheme: { light: { primary: { color: '#000' } } as never, dark: {} } } });
